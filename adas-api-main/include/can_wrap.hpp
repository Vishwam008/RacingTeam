...
namespace can
{
     ...
     * 
     * @throws can::error If socket creation failed.
     * @throws can::error If could not enable CANFD.
     * @throws can::error If could not bind socket.
     * 
     ...
     */
    int _connect( const bool fd, const std::string& channel )
    {
        const int canSocket = socket( PF_CAN, SOCK_RAW, CAN_RAW );
        if( canSocket < 0 )
            throw can::error( "Create socket failed" );
        ...
        if( fd && setsockopt( canSocket, SOL_CAN_RAW, CAN_RAW_FD_FRAMES,
            ...
            throw can::error( "Enable CAN FD failed" );

        if( bind( canSocket, reinterpret_cast<sockaddr*>(&addr), sizeof(addr) ) < 0 )
            throw can::error( "Bind socket failed for "+channel );
     ...
     */
    inline int connect( const std::string channel="vcan0" )
    {
     ...
     */
    inline int connectfd( const std::string channel="vcan0" )
    {
     ...
     * 
     * @throws can::error If read failed.
     * 
     * @param socket Socket identifer as returned from can::connect().
     ...
     * 
     * @throws can::error If write failed. 
     * 
